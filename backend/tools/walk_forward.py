...
def _returns(trades, lo, hi):
    ...
    for t in trades:
        ...
        if not ts or entry <= 0 or not (lo <= ts < hi):
            continue
        out.append(float(t.get("points") or 0) / entry * 100.0)
    return out
...
def main():
    ...
    for sym in spec["symbols"]:
        ...
        while cur + args.test * DAY_MS <= t1:
            ...
            te = _returns(runs[best_v], te_lo, te_hi)
            if len(te) >= MIN_TEST_TRADES:
                ...
                for v in spec["values"]:
                    fixed[sym][v] += _returns(runs[v], te_lo, te_hi)
